...
namespace ExpressionEditorExplorer.Samples
{
    ...
    public partial class TreeViewIntegration : UserControl
    {
        // index of the expression column
        private static int c_totalPriceIdx = 5;

        public TreeViewIntegration()
        {
            InitializeComponent();
        }

        private void TreeViewIntegration_Load(object sender, EventArgs e)
        {
            c1NWindDataSet.Tables.Add(DataSources.GetProducts());
            c1NWindDataSet.Tables.Add(DataSources.GetCategories());

            productsBindingSource.DataMember = "Products";
            productsBindingSource.DataSource = this.c1NWindDataSet;

            categoriesBindingSource.DataMember = "Categories";
            categoriesBindingSource.DataSource = this.c1NWindDataSet;

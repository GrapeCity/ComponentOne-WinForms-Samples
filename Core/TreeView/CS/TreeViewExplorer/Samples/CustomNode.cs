...
namespace TreeViewExplorer.Samples
{
    public class CategoryCustomNode : CustomContentPresenter
    {
        ...
        public override void SetValue(object value)
        {
            if (Node.Level == 0)
            {
                var row =  (System.Data.DataRow)Node.GetValue();
                _name.Text = row["CategoryName"]?.ToString();
                ...
                _img.Image = DataSource.Base64ToImage(row["Picture"]?.ToString());
                // set root panel
                Child = _rw;
            }
            else
    ...
    public class ProductCustomNode : CustomContentPresenter
    {
        ...
        public override void SetValue(object value)
        {
            ...
            {
                var row = (System.Data.DataRow)Node.GetValue();                
                _quantityPerUnit.Text = row["QuantityPerUnit"]?.ToString();
                _unitPrice.Text = decimal.Parse(row["UnitPrice"]?.ToString()).ToString("C");

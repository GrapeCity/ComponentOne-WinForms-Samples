...
namespace FlexGridExplorer.Samples
{
    public partial class TreeGrid : UserControl
    {
        ...
        private void PopulateGrid()
        {
            ...
            xdoc.LoadXml(Properties.Resources.TreeGroups);
        ...
        {
            Enum.GetValues(typeof(TreeStyleFlags)).Cast<TreeStyleFlags>().ToList().ForEach(x => cbStyle.Items.Add(x.ToString()));
            Enum.GetValues(typeof(System.Drawing.Drawing2D.DashStyle)).Cast<System.Drawing.Drawing2D.DashStyle>().ToList().ForEach(x => cbLineStyle.Items.Add(x.ToString()));
            Enum.GetValues(typeof(FocusRectEnum)).Cast<FocusRectEnum>().ToList().ForEach(x => cbFocusRect.Items.Add(x.ToString()));

            cbStyle.SelectedIndex = 3;
            cbLineStyle.SelectedIndex = 0;
            cbFocusRect.SelectedIndex = 3;
        }
        ...
        private void UpdateIcons()
        {
            for (int i = 1; i < flex.Rows.Count; i++)
            {
                if (flex.Rows[i].Node.Children < 1)
                {
                    ...
                    flex.SetCellImage(i, 0, imageList1.Images["Folder_closed.png"]);
            }
        }
        ...
        {
            flex.Tree.Style = (TreeStyleFlags)Enum.Parse(typeof(TreeStyleFlags), cbStyle.Text);
        }

        private void cbLineStyle_SelectedItemChanged(object sender, EventArgs e)
        {
            flex.Tree.LineStyle = (System.Drawing.Drawing2D.DashStyle)Enum.Parse(typeof(System.Drawing.Drawing2D.DashStyle), cbLineStyle.Text);
        }

        private void cbFocusRect_SelectedItemChanged(object sender, EventArgs e)
        {

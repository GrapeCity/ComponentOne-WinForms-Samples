...
namespace StockAnalysis
{
    ...
    public class EditableAnnotationLayer : AnnotationLayer
    {
        ...
        /// <summary>
        /// Default Style for newly added Annotations
        /// </summary>
        public ChartStyle DefaultStyle { get; set; }

        /// <summary>
        /// Default ContentStyle for newly added Annotations
        /// </summary>
        public ChartStyle DefaultContentStyle { get; set; }
        ...
        private void OnMouseMove(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            ...
            if (this.AllowAdd && this._start != PointF.Empty)
            {
                if (_drawing)
                {
                    this.UpdateAnnotation(e.Location);
                }
                else
                {
                    SizeF sz = new SizeF
        ...
        private bool ShowContentEditor(AnnotationBase anno, PointF location)
        {
            bool editorAvailable = ContentEditor != null;
            if (editorAvailable)
            {
        ...
        private void AddAnnotation(PointF p)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (pi != null) pi.SetValue(_annotationToAdd, pi.ReflectedType.Name, null);
                }
        ...
        private void UpdateAnnotation(PointF p)
        {
            _end = p;
    ...
    public class Helpers
    {
        ...
        {
            var annotation = anno as AnnotationBase;
            PointF point = PointF.Empty;
            _Size size;
            SizeF sz;
            if (annotation is Line)
                return GetRect(flexChart, annotation as Line);
            else if (annotation is Polygon)
                return GetRect(flexChart, annotation as Polygon);
            else
            {
                size = anno.GetSize();
                ...
                {
                    sz = TextRenderer.MeasureText(((Text)annotation).Content, ((Text)annotation).Style.Font);
                }
                PointF loc = AnnoPointToCoords(flexChart, annotation, annotation.Location);
    ...
    public static class Extensions
    {
        ...
        public static void ApplyStyle(this ChartStyle destStyle, ChartStyle sourceStyle)
        {
            if(sourceStyle.Fill != null) destStyle.Fill = sourceStyle.Fill;
            if(sourceStyle.Font != null) destStyle.Font = sourceStyle.Font;
            if(sourceStyle.Stroke != null) destStyle.Stroke = sourceStyle.Stroke;
            if(sourceStyle.StrokeDashPattern != null) destStyle.StrokeDashPattern = sourceStyle.StrokeDashPattern;
            if(sourceStyle.StrokeWidth >=0) destStyle.StrokeWidth = sourceStyle.StrokeWidth;
        }

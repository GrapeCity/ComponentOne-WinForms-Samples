...
namespace DataManipulation
{
    public abstract class FilterBase : INotifyPropertyChanged
    {
        ...
        {
            var v = Convert.ToDouble(obj.GetType().GetProperty(binding).GetValue(obj));
        ...
        {
            var values = from p in bindings select Convert.ToDouble(obj.GetType().GetProperty(p).GetValue(obj));

...
namespace SqlBuilder
{
	...
	public class QueryBuilder
	{
		...
		private string BuildFromClause()
		{
			...
			ArrayList qTables = new ArrayList();
			...
			// build join list
			ArrayList qJoins = new ArrayList();
			...
			for (int i = 0; i < qTables.Count-1; i++)
			{
				...
				sb.AppendFormat("\r\n\t({0} INNER JOIN", dt.TableName);
			}
			sb.AppendFormat(" {0}", ((DataTable)qTables[qTables.Count-1]).TableName);
		...
		private bool InsertRelatedTable(DataTable dt, ArrayList list)
		{
			...
			for (int index = 0; index <= list.Count; index++)
			{
				...
				bool before = index == 0 || 
					GetRelation(dt, (DataTable)list[index-1]) != null;

				// related to table after?
				bool after  = index == list.Count ||
					GetRelation(dt, (DataTable)list[index]) != null;
...
#if false
		// this is simpler than the BuildFromClause above, but doesn't work with Access.
		private string BuildFromClause2()
		{
			StringBuilder sb = new StringBuilder();
			...
			for (bool done = false; !done; )
			{
				...
				foreach (DataTable dt in tables)
				{
					// skip tables that have been added
					if (usedTables.Contains(dt))
						continue;
					...
					foreach (DataRelation dr in _schema.Relations)
					{
						if ((dr.ParentTable == dt && usedTables.Contains(dr.ChildTable)) ||
							(dr.ChildTable  == dt && usedTables.Contains(dr.ParentTable)))
						{
			...
			// not all tables joined? probably not what the user wants...
			_missingJoins = (usedTables.Count < tables.Count);

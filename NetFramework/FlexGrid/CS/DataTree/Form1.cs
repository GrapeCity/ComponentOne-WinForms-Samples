...
namespace DataTree
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		public Form1()
		{
			...
			// give all read-only columns a light gray background
            _flex.SetupColumns += (s, e) =>
                {
                    var flex = s as C1FlexGrid;
                    for (int i = flex.Cols.Fixed; i < flex.Cols.Count; i++)
                    {
                        if (!flex.Cols[i].AllowEditing)
                        {
                            flex.Cols[i].StyleNew.BackColor = Color.LightGray;
                        }
                    }
                };

            // check that BeforeCollapse and AfterCollapse events work
            _flex.BeforeCollapse += (s, e) =>
                {
                    Console.WriteLine("BeforeCollapse fired");
                };
            _flex.AfterCollapse += (s, e) =>
                {
                    Console.WriteLine("AfterCollapse fired");

                    // get the row that was expanded/collapsed
                    var row = _flex.Rows[e.Row];

                    // get the DataRowView on the parent grid
                    var drv = row.DataSource as DataRowView;

                    // get the child grid (if null, the node was collapsed)
                    var childGrid = row.UserData as C1FlexDataTree;

                    // get the data shown on the child grid
                    if (childGrid != null)
                    {
                        var childData = childGrid.DataSource;
                    }
                };
        }

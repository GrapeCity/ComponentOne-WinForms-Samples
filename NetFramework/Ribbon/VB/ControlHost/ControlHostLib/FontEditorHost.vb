...
Public Class FontEditorHost
    ...
    Public Const XmlNodeName As String = "fontEditorHost"
...
#Region "** methods"
    ...
    Protected Overrides Sub LoadControlHostProperties(ByVal node As XmlNode)
        ...
        Dim s As String = Read(node, "selectedFont", String.Empty)
        If s.Length = 0 Then
            SelectedFont = Nothing
        Else
            Dim tc As TypeConverter = TypeDescriptor.GetConverter(GetType(Font))
            SelectedFont = CType(tc.ConvertFromString(Nothing, CultureInfo.InvariantCulture, s), Font)
        End If
    End Sub
    ...
    Protected Overrides Sub SaveControlHostProperties(ByVal writer As XmlWriter)
        ...
        If SelectedFont IsNot Nothing Then
            Dim tc As TypeConverter = TypeDescriptor.GetConverter(GetType(Font))
            Dim s As String = tc.ConvertToString(Nothing, CultureInfo.InvariantCulture, SelectedFont)
            writer.WriteAttributeString("selectedFont", s)
        End If
    End Sub

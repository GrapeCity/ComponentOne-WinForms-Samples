...
namespace ControlHostLib
{
    ...
    {
        public const string XmlNodeName = "colorEditorHost";
        ...
        protected override void LoadControlHostProperties(XmlNode node)
        {
            ...
            string s = Read(node, "selectedColor", string.Empty);
            if (s.Length > 0)
            {
                TypeConverter tc = TypeDescriptor.GetConverter(typeof(Color));
                SelectedColor = (Color)tc.ConvertFromString(null, CultureInfo.InvariantCulture, s);
            }
        }
        ...
        protected override void SaveControlHostProperties(XmlWriter writer)
        {
            ...
            if (ShouldSerializeSelectedColor())
            {
                TypeConverter tc = TypeDescriptor.GetConverter(typeof(Color));
                string s = tc.ConvertToString(null, CultureInfo.InvariantCulture, SelectedColor);
    ...
    /// <summary>
    /// Uses the Color UITypeEditor to show a dropdown where users can pick colors.
    /// </summary>
    public class ColorEditorControl : ComboBox, IServiceProvider, IWindowsFormsEditorService
    {
        ...
        // form used to show the dropdown
        private Form _form;

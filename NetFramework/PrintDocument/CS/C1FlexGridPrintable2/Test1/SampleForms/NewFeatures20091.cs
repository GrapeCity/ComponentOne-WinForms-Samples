...
namespace NewFeatures20091
{
    public partial class Form1 : Form, C1FlexGridPrinterTest.IFlexSample
    {
        ...
        void _flexRowValidation_RowValidating(object sender, RowColEventArgs e)
        {
            ...
            {
                string catName = (string)_flexRowValidation[e.Row, "CategoryName"];
                switch (catName.Substring(0, 1))
                {
                    case "A":
                    case "B":
                    case "C":
                    case "D":
                        break;
                    default:
                        MessageBox.Show("Category names must start with 'A', 'B', 'C', or 'D'.", "RowValidating failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        e.Cancel = true;
                        break;
                }

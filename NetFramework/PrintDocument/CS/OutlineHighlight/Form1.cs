...
namespace OutlineHighlight
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            // find outline view, attach an event handler if found:
            C1PreviewOutlineView outline = FindControl(c1PrintPreviewControl1, typeof(C1PreviewOutlineView)) as C1PreviewOutlineView;
            if (outline != null)
                outline.AfterSelect += new TreeViewEventHandler(outline_AfterSelect);
        ...
        static private Control FindControl(Control parent, Type type)
        {
            foreach (Control c in parent.Controls)
            {
                if (type.IsAssignableFrom(c.GetType()))
                    return c;
                Control ctrl = FindControl(c, type);
                if (ctrl != null)
                    return ctrl;
            }
            return null;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            for (int row = 0; row < 100; ++row)
                for (int col = 0; col < 4; ++col)
                {
                    ...
                    doc.Outlines.Add(rt.Cells[row, col].Text, rt.Cells[row, col].RenderObject);//qq
                }

...
namespace DashboardModel
{
    public class DataService
    {
...
#endif
        public delegate IEnumerable GetRegionWiseSaleCollectionDelegate();
        public GetRegionWiseSaleCollectionDelegate GetRegionWiseSaleCollection;
...
#endif
        {
            ...
            var saleList = dashboardContext.Sales
                .Include(s => s.Product).ThenInclude(p => p.Category)
                .Include(s => s.Customer).ThenInclude(c => c.Region)
                .Include(s => s.Campaign).ToList();
            ...
            foreach (Sale sale in saleCollection)
            {
                ...
                productsWiseSale.Sales = sale.Summ;
        ...
        public IEnumerable GetRegionWiseSalesWithCategory()
        {
                return from sale in regionWiseSaleCollection.Cast<RegionWiseSale>()
                       join prod in productCollection.Cast<Product>() on sale.Product equals prod.Name
                       select new RegionWiseSaleCategory() { Product = sale.Product, RegionId = sale.RegionId, X = sale.X, Y = sale.Y, Sales = sale.Sales, Profit = sale.Profit, Region = sale.Region, Category = prod.Category.Name };
        }

        public List<RegionSaleItem> GetRegionWiseSales()
        {
            var regionSaleMap = new Dictionary<int, RegionSaleItem>();
            foreach (RegionWiseSale regionWiseSale in GetRegionWiseSaleCollection != null ? GetRegionWiseSaleCollection() : GetRegionWiseSalesWithCategory())
            {
                ...
                else
                {
                    ...
                    regionSaleMap.Add(regionWiseSale.RegionId, new RegionSaleItem { Id = regionWiseSale.RegionId, Locat = new Point(regionWiseSale.X, regionWiseSale.Y), Sales = regionWiseSale.Sales, Profit = regionWiseSale.Profit, Name = regionWiseSale.Region,
                        Category = (regionWiseSale as RegionWiseSaleCategory)?.Category});
#endif
                }
            }

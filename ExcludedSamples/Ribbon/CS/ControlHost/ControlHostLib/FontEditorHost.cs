...
namespace ControlHostLib
{
    ...
    {
        public const string XmlNodeName = "fontEditorHost";
        ...
        protected override void LoadControlHostProperties(XmlNode node)
        {
            ...
            string s = Read(node, "selectedFont", string.Empty);
            if (s.Length == 0)
                SelectedFont = null;
            else
            {
                TypeConverter tc = TypeDescriptor.GetConverter(typeof(Font));
                SelectedFont = (Font)tc.ConvertFromString(null, CultureInfo.InvariantCulture, s);
            }
        }
        ...
        protected override void SaveControlHostProperties(XmlWriter writer)
        {
            ...
            {
                TypeConverter tc = TypeDescriptor.GetConverter(typeof(Font));
                string s = tc.ConvertToString(null, CultureInfo.InvariantCulture, SelectedFont);
